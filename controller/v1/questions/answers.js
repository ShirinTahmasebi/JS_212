...
class AnswerController {
  async post(request, response, next) {
    ...
    if (question_type === question_types.SIMPLE && answer_text) {
      [validate_answer_error, validate_answer_response] = await AnswerController.validate_answer_to_simple_question(request, response);
    } else if (question_type === question_types.MULTI_CHOICE && answer_choices_ids) {
      [validate_answer_error, validate_answer_response] = await AnswerController.validate_answer_to_multichoice_question(request, response);
    } else {
  ...
  static async validate_answer_to_multichoice_question(request, response) {
    const question_id = request.params.question_id;
    const answer_choices_ids = request.body.answer_choices_ids;
  ...
  static async validate_answer_to_simple_question(request, response) {
    const question_id = request.params.question_id;
    const answer_text = request.body.answer_text;
