...
const queries = {
  mysql: {
    questions: {
      ...
      get_random_question: 'SELECT * FROM questions WHERE question_id ORDER BY RAND() LIMIT 1',
      get_random_question_except_prev_questions: 'SELECT * FROM questions WHERE question_id NOT IN ({previous_question_ids}) ORDER BY RAND() LIMIT 1',
      get_random_simple_question: `SELECT * FROM questions WHERE question_type=${question_types.SIMPLE} ORDER BY RAND() LIMIT 1`,
      get_random_simple_question_except_prev_questions: `SELECT * FROM questions WHERE question_type=${question_types.SIMPLE} AND question_id NOT IN ({previous_question_ids}) ORDER BY RAND() LIMIT 1`,
      ...
      get_random_multi_choice_question_except_prev_questions: `SELECT * FROM questions WHERE question_type=${question_types.MULTI_CHOICE} AND question_id NOT IN ({previous_question_ids}) ORDER BY RAND() LIMIT 1`,
